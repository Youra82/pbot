...
def main():
    ...
    bot_runner_script = os.path.join(SCRIPT_DIR, 'src', 'pbot', 'strategy', 'run.py')
    ...
    try:
        ...
        # *** KORREKTUR: Sucht jetzt nach 'pbot' ***
        if not secrets.get('pbot'): 
            print("Fehler: Kein 'pbot'-Account in secret.json gefunden.")
            return
        main_account_config = secrets['pbot'][0] # Nutzt 'pbot'
        ...
        for strategy_info in strategy_list:
            ...
            if use_autopilot and isinstance(strategy_info, str):
                ...
                # use_macd wird nicht mehr benötigt, aber wir müssen einen Dummy-Wert übergeben
