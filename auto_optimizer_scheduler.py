...
def run_optimization_python() -> bool:
    ...
    optimizer_path = SCRIPT_DIR / "src" / "pbot" / "analysis" / "optimizer.py"
    ...
    try:
        ...
        if returncode == 0:
            ...
            if opt_settings.get("send_telegram_on_completion", True):
                config_dir = SCRIPT_DIR / "src" / "pbot" / "strategy" / "configs"
                config_count = len(list(config_dir.glob("config_*.json"))) if config_dir.exists() else 0
                
                send_telegram(f"""✅ PBot Auto-Optimierung ABGESCHLOSSEN
...
Lookback: {lookback_days} Tage""")
            ...
            if opt_settings.get("send_telegram_on_completion", True):
                send_telegram(f"""❌ PBot Auto-Optimierung FEHLGESCHLAGEN
