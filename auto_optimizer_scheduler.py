...
OPTIMIZER_SCRIPT = os.path.join(PROJECT_ROOT, 'src', 'pbot', 'analysis', 'optimizer.py')
...
def _send_start_telegram(pair_display: list, num_trials: int, start_time: datetime):
    msg = (
        f"\U0001f680 PBot Auto-Optimizer GESTARTET\n"
...
def _send_end_telegram(elapsed_seconds: float):
    ...
    if not os.path.exists(OPTIMIZER_RESULTS_FILE):
        _send_telegram_plain(f"\u2705 PBot Auto-Optimizer abgeschlossen\nDauer: {dur}")
        return
    ...
    try:
        ...
        _send_telegram_plain(f"\u2705 PBot Auto-Optimizer abgeschlossen (Dauer: {dur})")
        return
    ...
    lines = [f"\u2705 PBot Auto-Optimizer abgeschlossen (Dauer: {dur})"]
...
def main():
    parser = argparse.ArgumentParser(description='PBot Auto-Optimizer Scheduler')
