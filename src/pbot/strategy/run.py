# /root/pbot/src/pbot/strategy/run.py
import os
import sys
import json
import logging
...
from pbot.utils.trade_manager import full_trade_cycle
from pbot.utils.timeframe_utils import determine_htf # NEU: Import für HTF Bestimmung

def setup_logging(symbol, timeframe):
    safe_filename = f"{symbol.replace('/', '').replace(':', '')}_{timeframe}"
    log_dir = os.path.join(PROJECT_ROOT, 'logs')
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, f'pbot_{safe_filename}.log')

    logger = logging.getLogger(f'pbot_{safe_filename}')
...
def main():
    ...
    try:
        ...
        # Lese Account-Konfigurationen (nach der Umbenennung erwarten wir 'pbot' Schlüssel)
        accounts_to_run = secrets.get('pbot', [])
        if not accounts_to_run:
            logger.critical("Keine Account-Konfigurationen unter 'pbot' in secret.json gefunden!")
            sys.exit(1)
    ...
    if not isinstance(accounts_to_run, list):
        logger.critical("Fehler: 'pbot'-Eintrag in secret.json ist keine Liste von Accounts.")
        sys.exit(1)
